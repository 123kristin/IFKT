...
class MYMODEL(nn.Module):
    def __init__(self, n_question, n_qtype, max_seq, embed_dim, n_query_features, n_gcn_layers, gcn_data, dropout, new_att=False):
        ...
        self.n_query_features_q = 2
        # self.n_query_features_x = n_query_features - 3
        ...
        # q拼接4个特征 e, s, qt, d
        # self.combine_q_embedding_q = nn.Linear(embed_dim * (n_query_features - 2), embed_dim)
        # self.q_feature_layer_norm_q = nn.LayerNorm([max_seq-1, embed_dim]) # 对每一个batch进行归一化
        # x拼接3个特征 x, attempt, ms
        # self.combine_q_embedding_x = nn.Linear(embed_dim * (n_query_features - 3), embed_dim)
        # self.q_feature_layer_norm_x = nn.LayerNorm([max_seq-1, embed_dim]) # 对每一个batch进行归一化

        # q拼接4个特征 e, s, qt, d
        self.combine_q_embedding_q = nn.Linear(embed_dim * (n_query_features), embed_dim)
        self.q_feature_layer_norm_q = nn.LayerNorm([max_seq-1, embed_dim]) # 对每一个batch进行归一化
    ...
    def forward(self, x, question_ids, skill_ids, qt_ids, diff, ms_response_norm):
        ...
        # combine_list_q = []
        # bs, seq_len, embed_dim
        # combine_embed_q = torch.cat((e, s, qt, d), dim=2)
        # for i in range(combine_embed_q.shape[0]):
        #     tmp = self.combine_q_embedding_q(combine_embed_q[i, :, :].float()).unsqueeze(0)
        #     combine_list_q.append(tmp)
        # q_in = torch.cat(combine_list_q, dim=0)
        # del combine_list_q
        # q_in = self.q_feature_layer_norm_q(q_in.permute(1, 0, 2)).permute(1, 0, 2)

        # # x拼接3个特征 x, attempt, ms
        # combine_list_x = []
        # # bs, seq_len, embed_dim
        # combine_embed_x = torch.cat((x, attempt, ms), dim=2)
        # for i in range(combine_embed_x.shape[0]):
        #     tmp = self.combine_q_embedding_x(combine_embed_x[i, :, :].float()).unsqueeze(0)
        #     combine_list_x.append(tmp)
        # x_in = torch.cat(combine_list_x, dim=0)
        # del combine_list_x
        # x_in = self.q_feature_layer_norm_x(x_in.permute(1, 0, 2)).permute(1, 0, 2)

        # 拼接2个特征：e, s
        combine_list_q = []
        # bs, seq_len, embed_dim
        combine_embed_q = torch.cat((e, s), dim=2)
        for i in range(combine_embed_q.shape[0]):
            tmp = self.combine_q_embedding_q(combine_embed_q[i, :, :].float()).unsqueeze(0)
            combine_list_q.append(tmp)
        q_in = torch.cat(combine_list_q, dim=0)
        ...
        return x.squeeze(-1), att_weight, gcn_e, gcn_s, W_Q, W_S  
...
class MYModule(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # output, _, _, _, _, _ = self(x, target_qid, target_sid, target_qtype, target_qd, target_qms, target_qattempt)
        output, _, _, _, _, _ = self(x, target_qid, target_sid, target_qtype, target_qd, target_qms)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # output, _, _, _, _, _ = self(x, target_qid, target_sid, target_qtype,
